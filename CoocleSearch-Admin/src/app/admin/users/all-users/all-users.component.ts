...
export class AllUsersComponent implements OnInit{
  ...
  Readalluser(){
      this.http.ReadAllUser().subscribe({
        next: (response: any) => {
          this.allUsers = response || []; // Stocker les produits dans le tableau
          if(response?.users)  {
  ...
  // addUser(){
  //   let body = this.user_form.value
  //   this.http.CreateUser(body).subscribe({
  //     next:(res: any) =>{
  //       this.user_data = res?.user_info
  //       this.allSupervisors()
  ...
  //     console.log(err);
  //     this.user_form.reset()
  //     this.showErrorToast('Failed to create user.')
