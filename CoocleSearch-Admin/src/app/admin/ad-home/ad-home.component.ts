...
export class AdHomeComponent implements OnInit {
  ...
  loadCategories() {
    this.http.ReadAllCategories()?.subscribe({
      next: (res:any) =>{
          this.dataCategorie = res?.categorie
  ...
  loadBusiness() {
    this.http.ReadAllBusiness()?.subscribe({
      next: (res:any) =>{
          this.dataBusiness = res?.business
  ...
  loadAdvertisement() {
    this.http.ReadAllAdvertisement()?.subscribe({
      next: (res:any) =>{
          this.dataAdvertisement = res?.advertisement
          console.log(this.dataAdvertisement)
      }
    });
  }
